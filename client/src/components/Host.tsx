...
export const Host = (
  ...
  const onInitialState = useCallback((_redClicks: number, _greenClicks: number, _redCPS: number, _greenCPS: number, gameSetup: InitSetup) => {
    ...
    setTempGameMode(gameSetup.gameMode);
    switch (gameSetup.gameMode) {  // gameMode may not have updated yet so can't use the state
      case "Most in Time":
        setTempTimer(gameSetup.time);
        setTimer(gameSetup.time);
        break;
      case "First to Target":
        setTempTarget(gameSetup.target);
        setTarget(gameSetup.target);
        break;
      case "Neverending":
        break;  // for consistency and future development
    }
    setLoading(false);
  }, []);
  ...
  return (
    ...
    `}>
            ...
            <div className={`${styles["flex"]} ${styles["flex-col"]} ${styles["gap-4"]}`}>
              {
                ...
                ? (
                  <div className={`${TW_SETTINGS_CONT}`}>
                    <span>Target</span>
                    <input
                      type="range" min={100} max={10000} value={Math.round(Math.pow(tempTarget, 0.5)*100)} className={`${styles["range"]}`}
                      onChange={(e) => { setTempTarget(Math.ceil(Math.pow(parseInt(e.target.value) / 100, 2))); }}
                    />
...

      {
        ...
        `} style={
            medsz
            ? { left: `${getRedSizeFactor() * 100}%` }
            : { top: `${getRedSizeFactor() * 100}%`}
          }
        >
          {timer}
        </div>) : null
